...
export const FeesAccountsView: React.FC = () => {
  ...
  const filteredStudents = students.filter(s => {
    const searchMatch =
      ...
      (s.studentCode || s.studentId).toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const statusMatch =
      ...
      (statusFilter === 'pending' && s.feesPending > 0 && s.feesOverdue === 0) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Multi-branch fee reconciliation, installment schedules, and GST receipts.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-slate-500">Total Course Fee Billing</span>
          <div className="mt-1 text-2xl font-black text-slate-900">₹{(totalBilled / 100000).toFixed(2)}L</div>
          <span className="text-[10px] text-slate-400">Total contracted fee pool</span>
        </div>

        <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-emerald-800">Total Collected</span>
          <div className="mt-1 text-2xl font-black text-emerald-900">₹{(totalCollected / 100000).toFixed(2)}L</div>
          <span className="text-[10px] font-bold text-emerald-700">Realized: {recoveryRate}%</span>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-slate-500">Scheduled Balance</span>
          <div className="mt-1 text-2xl font-black text-slate-800">₹{(totalPending / 100000).toFixed(2)}L</div>
          <span className="text-[10px] text-slate-400">Future installments</span>
        </div>

        <div className="rounded-xl border border-red-200 bg-red-50/50 p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-red-800">Overdue Recovery Queue</span>
          <div className="mt-1 text-2xl font-black text-red-900">₹{(totalOverdue / 100000).toFixed(2)}L</div>
          <span className="text-[10px] font-bold text-red-700">Past due date</span>
        </div>
      </div>
              ...
              {filteredStudents.slice(0, 15).map(student => (
                <tr key={student.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="py-3 px-4">
                    ...
                    <div className="text-[10px] text-slate-500">{student.batchName}</div>
                  </td>

                  <td className="py-3 px-4">
                    <span className={`inline-block rounded px-2 py-0.5 text-[10px] font-bold ${
                      student.scholarshipPercent > 0 ? 'bg-indigo-100 text-indigo-800' : 'bg-slate-100 text-slate-600'
                    }`}>
                  ...
                  <td className="py-3 px-4 text-right">
                    {student.feesPending === 0 ? (
                      ...
                      <span className="text-red-700 font-black">₹{student.feesOverdue.toLocaleString()} (Overdue)</span>
                    ) : (
                  ...
                  <td className="py-3 px-4 text-center">
                    <span className={`inline-block rounded px-2 py-0.5 text-[10px] font-bold ${
                      ...
                      {student.feesPending === 0 ? 'CLEARED' : student.feesOverdue > 0 ? 'OVERDUE' : 'ON SCHEDULE'}
                    </span>
                  </td>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-xs">
        <h3 className="text-sm font-bold text-slate-900 mb-1">Recent Computerized Fee Receipts</h3>
        <p className="text-xs text-slate-500 mb-4">Official receipts signed with institute transaction stamp</p>
            ...
            <tbody className="divide-y divide-slate-100">
              {feeReceipts
                ...
                <tr key={rcpt.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="py-2.5 px-4 text-right font-black text-emerald-800">₹{rcpt.amount.toLocaleString()}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl border border-slate-300 relative">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 print:hidden">
              <span className="text-xs font-bold text-slate-600">Official Computerized Cash Receipt</span>
              <div className="flex items-center gap-2">
                <button
            ...
            <div className="mt-4 p-4 border border-slate-200 rounded-xl bg-slate-50/40 space-y-4">
              ...
              <div className="text-center border-b border-slate-200 pb-3">
                ...
                <h2 className="text-base font-black tracking-tight text-slate-900">CAREER HEIGHTS</h2>
                <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">
                  CENTRALIZED MULTI-BRANCH EDUCATION ERP
                </p>
                <p className="text-[10px] text-slate-400">Head Office: Main Chowk, Handwara, J&amp;K | Reg: CH-EDU-2021</p>
              </div>
              ...
              <div className="flex justify-between text-xs font-semibold py-1">
                <div>
                  ...
                  <span className="font-mono text-blue-900 font-bold">{receiptToPrint.receiptNo}</span>
                </div>
                <div>
              ...
              <div className="rounded-lg bg-white p-3 border border-slate-200 text-xs space-y-1.5">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-bold text-slate-800">{receiptToPrint.studentCode}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-600 italic">{receiptToPrint.notes || 'Tuition Fee Payment'}</span>
                </div>
              </div>
              ...
              <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-3.5 text-center">
                ...
                <p className="text-2xl font-black text-emerald-900">₹{receiptToPrint.amount.toLocaleString()}</p>
                <p className="text-[10px] text-emerald-700">Official Computer-Generated Acknowledgment</p>
              </div>
              ...
              <div className="pt-4 flex justify-between items-end text-[10px] text-slate-400">
                <div>
                  ...
                  <p className="font-semibold text-slate-700">{receiptToPrint.receivedBy}</p>
                </div>
                <div className="text-right">
                  <p className="border-t border-slate-300 pt-1 font-semibold text-slate-600">Seal of Career Heights</p>
                </div>
              </div>
            </div>
          </div>
