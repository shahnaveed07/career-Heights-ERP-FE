...
export const StudentPortalView = () => {
  ...
  const { students, testResults, submitDoubt, doubts, timetable } =
    useErpData();
  const student =
  ...
  const handleAskDoubt = (e) => {
    ...
    setTimeout(() => setDoubtSuccess(false), 3e3);
  };
  if (!student) {
    return (
            ...
            <div className="rounded-xl bg-white/10 p-3 text-center backdrop-blur-xs">
              ...
              <span className="text-xl font-black text-white">
                {student.feesPending === 0
                  ? 'NIL'
                  : `\u20B9${(student.feesPending / 1e3).toFixed(0)}k`}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
          ...
          <p className="mt-1 text-xs text-slate-500">
            82 of 88 Class Days Present (Regular)
          </p>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Score: {myResults[0]?.totalMarksObtained || 645} /{' '}
            {myResults[0]?.totalMaxMarks || 720} (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Paid: ₹{student.feesPaid.toLocaleString()} of ₹
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs">
            ...
            <p className="text-xs text-slate-500 mb-3">
              Direct line to senior faculty. Answers will appear below with
              detailed steps.
            </p>
            ...
            {myDoubts.map((d) => (
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`rounded px-1.5 py-0.2 text-[9px] font-bold ${d.status === 'answered' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}
                  >
