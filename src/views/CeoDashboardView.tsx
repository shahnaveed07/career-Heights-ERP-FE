...
export const CeoDashboardView: React.FC<CeoDashboardViewProps> = ({
  ...
  // Level: 'hq' | 'branch' | 'wing' | 'class' | 'batch' | 'student'
  const [drillLevel, setDrillLevel] = useState<'hq' | 'branch' | 'wing' | 'class' | 'batch'>('hq');
  const [selectedBranchId, setSelectedBranchId] = useState<string | null>(null);
  const [selectedWingId, setSelectedWingId] = useState<string | null>(null);
  const [selectedClassId, setSelectedClassId] = useState<string | null>(null);
  const [selectedBatchId, setSelectedBatchId] = useState<string | null>(null);

  // Filter students based on branch switcher if not 'all'
  ...
  const criticalAcademicStudents = filteredStudents.filter(s => s.academicRisk === 'Critical');
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-500">Live Consolidated View</span>
            </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            ...
            <p className="mt-1 text-[10px] text-slate-400">Enrolled across 5 branches</p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            <span className="text-[11px] font-semibold text-slate-500">Conversion %</span>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-2xl font-black text-emerald-700">{admissionConversionRate}%</span>
              <span className="text-[10px] font-semibold text-slate-500">Target: 30%</span>
            </div>
            <p className="mt-1 text-[10px] text-slate-400">Lead-to-enrollment ratio</p>
          </div>
          ...
          {/* 6. Outstanding Fees */}
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            <span className="text-[11px] font-semibold text-slate-500">Outstanding Fees</span>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-2xl font-black text-red-700">₹{(outstandingFees / 100000).toFixed(2)}L</span>
              <span className="flex items-center text-[10px] font-bold text-amber-700">
                <AlertTriangle className="h-3 w-3" /> Due
              </span>
            </div>
            <p className="mt-1 text-[10px] text-slate-400">Installments pending</p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            ...
            <div className="mt-1 flex items-baseline justify-between">
              ...
              <span className="text-[10px] font-semibold text-emerald-700">&gt;85% Standard</span>
            </div>
            <p className="mt-1 text-[10px] text-slate-400">Real-time daily average</p>
          </div>

          {/* 8. Tests Conducted */}
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            <span className="text-[11px] font-semibold text-slate-500">Tests Conducted</span>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-2xl font-black text-blue-900">{totalTestsConducted}</span>
              <span className="text-[10px] font-bold text-blue-700">OMR + Online</span>
            </div>
            <p className="mt-1 text-[10px] text-slate-400">Evaluated &amp; processed</p>
          </div>

          {/* 9. Active Faculty */}
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            <span className="text-[11px] font-semibold text-slate-500">Active Faculty</span>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-2xl font-black text-slate-900">{activeFacultyCount}</span>
              <span className="text-[10px] font-bold text-emerald-700">100% Present</span>
            </div>
            <p className="mt-1 text-[10px] text-slate-400">Academic staff on duty</p>
          </div>

          {/* 10. Pending Documents */}
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5">
            <span className="text-[11px] font-semibold text-slate-500">Pending Documents</span>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-2xl font-black text-amber-600">{pendingDocsCount}</span>
              <span className="text-[10px] font-bold text-amber-700">Needs Audit</span>
            </div>
            <p className="mt-1 text-[10px] text-slate-400">Verification vault queue</p>
          </div>
        </div>
      </div>
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              ...
              <span className="rounded-full bg-red-100 px-2.5 py-0.5 text-[10px] font-bold text-red-700">
                {criticalAttendanceStudents.length + overdueFeeStudents.length + criticalAcademicStudents.length} Critical Action Items
              </span>
            </h2>
            <p className="text-xs text-slate-500">
              Automated algorithmic flags triaged into Red, Amber, and Green management tiers.
            </p>
          </div>
        </div>
          ...
          <div className="rounded-xl border border-red-200 bg-red-50/50 p-4">
            <div className="flex items-center justify-between pb-2 border-b border-red-200">
              ...
              <span className="rounded-full bg-red-600 px-2 py-0.5 text-[10px] font-extrabold text-white">
                {criticalAttendanceStudents.length + overdueFeeStudents.length} Items
              </span>
            </div>
            ...
            <div className="mt-3 space-y-2.5 text-xs">
              ...
              <div className="rounded-lg bg-white p-2.5 border border-red-100 shadow-2xs">
                <div className="flex items-center justify-between font-bold text-red-900">
                  ...
                  <span className="text-red-700">{criticalAttendanceStudents.length} Students</span>
                </div>
                <p className="mt-1 text-[11px] text-slate-600">
                  e.g. <strong>Faizan Dar</strong> (Handwara JEE-A) at <strong>64.2%</strong>. Absent for 4 tests.
                </p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-[10px] text-slate-400">Auto SMS triggered to parent</span>
                  <button
              ...
              <div className="rounded-lg bg-white p-2.5 border border-red-100 shadow-2xs">
                <div className="flex items-center justify-between font-bold text-red-900">
                  ...
                  <span className="text-red-700">{overdueFeeStudents.length} Cases</span>
                </div>
                <p className="mt-1 text-[11px] text-slate-600">
                  Totaling <strong>₹{overdueFeeStudents.reduce((a, b) => a + b.feesOverdue, 0).toLocaleString()}</strong> pending beyond due date.
                </p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-[10px] text-slate-400">Notice issued</span>
                  <button
                    onClick={() => onNavigateToModule?.('fees')}
                    className="text-[11px] font-bold text-red-700 hover:underline"
                  >
              ...
              <div className="rounded-lg bg-white p-2.5 border border-red-100 shadow-2xs">
                <div className="flex items-center justify-between font-bold text-red-900">
                  ...
                  <span className="text-red-700">{criticalAcademicStudents.length} Students</span>
                </div>
                <p className="mt-1 text-[11px] text-slate-600">
                  Requiring remedial batch allocation and weekly mentor 1-on-1 check-ins.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-4">
            <div className="flex items-center justify-between pb-2 border-b border-amber-200">
              ...
              <span className="rounded-full bg-amber-600 px-2 py-0.5 text-[10px] font-extrabold text-white">
                {lowAttendanceStudents.length + pendingDocsCount + followUpDueEnquiries.length} Items
              </span>
            </div>
            ...
            <div className="mt-3 space-y-2.5 text-xs">
              ...
              <div className="rounded-lg bg-white p-2.5 border border-amber-100 shadow-2xs">
                <div className="flex items-center justify-between font-bold text-amber-900">
                  ...
                  <span className="text-amber-800">{lowAttendanceStudents.length} Students</span>
                </div>
                <p className="mt-1 text-[11px] text-slate-600">
                  Approaching minimum threshold. Faculty mentors notified to issue early warning.
                </p>
              </div>
              ...
              <div className="rounded-lg bg-white p-2.5 border border-amber-100 shadow-2xs">
                <div className="flex items-center justify-between font-bold text-amber-900">
                  ...
                  <span className="text-amber-800">{pendingDocsCount} Docs</span>
                </div>
                <p className="mt-1 text-[11px] text-slate-600">
                  Unverified Aadhaar cards and blurred Class 10 marksheet scans in vault.
                </p>
              ...
              <div className="rounded-lg bg-white p-2.5 border border-amber-100 shadow-2xs">
                <div className="flex items-center justify-between font-bold text-amber-900">
                  ...
                  <span className="text-amber-800">{followUpDueEnquiries.length} Leads</span>
                </div>
                <p className="mt-1 text-[11px] text-slate-600">
                  Includes top CHTQ scorers pending seat confirmation at Handwara &amp; Qaziabad.
                </p>
                <div className="mt-2 text-right">
                  <button
                    onClick={() => onNavigateToModule?.('admissions_crm')}
          ...
          <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4">
            ...
            <div className="mt-3 space-y-2.5 text-xs">
              <div className="rounded-lg bg-white p-2.5 border border-emerald-100 shadow-2xs">
                ...
                <p className="mt-1 text-[11px] text-slate-600">
                  All 30+ faculty and staff logged biometric checks. Syllabus running on schedule.
                </p>
              </div>
              ...
              <div className="rounded-lg bg-white p-2.5 border border-emerald-100 shadow-2xs">
                ...
                <p className="mt-1 text-[11px] text-slate-600">
                  Strong fee recovery in Q2 tuition installments across top cohorts.
                </p>
              </div>
              ...
              <div className="rounded-lg bg-white p-2.5 border border-emerald-100 shadow-2xs">
                ...
                <p className="mt-1 text-[11px] text-slate-600">
                  JEE Advanced mock tests showing steady upward percentile migration.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-200 pb-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400">Click any level to drill down</span>
            </div>
            <h2 className="mt-1 text-lg font-bold text-slate-900">
              Drill-down: HQ &rarr; Branch &rarr; Wing &rarr; Class &rarr; Batch &rarr; Student
            </h2>
          </div>
          ...
          {drillLevel === 'hq' && (
            <div>
              <div className="flex items-center justify-between mb-3 text-xs text-slate-500">
                <span>Select a branch below to explore its academic wings and cohorts:</span>
                <span className="font-semibold text-slate-700">5 Operational Campuses</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
                {branches.map(b => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-500">{b.city} Campus</p>

                    <div className="mt-3 pt-3 border-t border-slate-200 space-y-1 text-xs">
                      <div className="flex justify-between">
                        <span className="text-slate-500">Students:</span>
                        <span className="font-bold text-slate-900">{b.studentCount} / {b.capacity}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Attendance:</span>
                        <span className="font-semibold text-emerald-700">{b.attendanceRate}%</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Revenue:</span>
                        <span className="font-bold text-slate-900">₹{(b.monthlyRevenue / 100000).toFixed(2)}L</span>
                      </div>
                    </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3 text-xs">
                <div>
                  ...
                  <strong className="text-slate-900 text-sm">{activeBranch.name}</strong>
                  <span className="ml-2 text-slate-400">({activeBranch.address})</span>
                </div>
                <button
                  onClick={resetToHq}
                ...
                {branchWings.map(w => (
                  ...
                  >
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                      <span className="text-slate-500 font-medium">Classes enrolled in wing</span>
                      <span className="font-bold text-blue-900">View Classes &rarr;</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3 text-xs">
                <div>
                  ...
                  <strong className="text-slate-900 text-sm">{activeWing.name}</strong>
                  <span className="ml-2 text-slate-400">in {activeBranch?.name}</span>
                </div>
                <button
                  onClick={() => setDrillLevel('branch')}
                  className="text-xs font-semibold text-blue-900 hover:underline"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {wingClasses.map(c => (
                  ...
                  >
                    ...
                    <p className="text-xs text-blue-900 font-semibold mt-0.5">{c.stream}</p>
                    <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                      <span className="text-slate-500">Assigned Batches</span>
                      <span className="font-bold text-blue-900">Select Batch &rarr;</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3 text-xs">
                <div>
                  ...
                  <strong className="text-slate-900 text-sm">{activeClass.name}</strong>
                </div>
                <button
                  onClick={() => setDrillLevel('wing')}
                  className="text-xs font-semibold text-blue-900 hover:underline"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {classBatches.map(b => (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500">Mentor: {b.facultyMentor}</p>

                    <div className="mt-3 space-y-1 text-xs text-slate-600">
                      <div className="flex justify-between">
                        <span>Timing:</span>
                        <span className="font-semibold text-slate-800">{b.timing}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Students:</span>
                        <span className="font-bold text-slate-900">{b.studentCount} / {b.capacity}</span>
              ...
              <div className="flex items-center justify-between mb-3 text-xs">
                <div>
                  ...
                  <span className="ml-2 text-slate-500">
                    Mentor: {activeBatch.facultyMentor} • {batchStudents.length} Students
                  </span>
                </div>
                <button
                  onClick={() => setDrillLevel('class')}
                      ...
                      <tr key={s.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="py-2.5 px-3">
                          <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${
                            s.scholarshipPercent > 0 ? 'bg-indigo-100 text-indigo-800' : 'bg-slate-100 text-slate-600'
                          }`}>
                            {s.scholarshipType}
                          </span>
                        </td>
                        <td className="py-2.5 px-3">
                          <span className={`font-bold ${
                            s.attendanceRate < 75 ? 'text-red-700' : s.attendanceRate < 80 ? 'text-amber-700' : 'text-emerald-700'
                          }`}>
                            {s.attendanceRate}%
                          </span>
                        </td>
                        <td className="py-2.5 px-3">
                          {s.feesPending === 0 ? (
                            ...
                            <span className="text-red-700 font-bold">₹{s.feesOverdue.toLocaleString()} Overdue</span>
                          ) : (
                            <span className="text-slate-600">₹{s.feesPending.toLocaleString()} Pending</span>
                          )}
                        </td>
                        <td className="py-2.5 px-3">
                          <span className={`rounded-full px-2 py-0.5 text-[9px] font-extrabold ${
                            s.academicRisk === 'Critical'
                              ? 'bg-red-100 text-red-800'
                              : s.academicRisk === 'Medium'
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Cross-branch benchmarking of enrollments, revenue, collections, and academic attendance.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-slate-900">Monthly Admissions &amp; Enquiries</h3>
              <p className="text-xs text-slate-500">6-Month Trend across Academic Year 2025-26</p>
            </div>
          ...
          <div className="mt-3 flex items-center justify-center gap-6 text-xs">
            <div className="flex items-center gap-1.5">
              <span className="h-3 w-3 rounded-xs bg-blue-900"></span>
              <span className="text-slate-600 font-medium">Confirmed Admissions</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="h-3 w-3 rounded-xs bg-amber-400"></span>
              <span className="text-slate-600 font-medium">Walk-in &amp; Online Leads</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-slate-900">Monthly Revenue &amp; Collection (₹ Lakhs)</h3>
              <p className="text-xs text-slate-500">Scheduled vs Realized tuition payments</p>
            </div>
          ...
          <div className="mt-3 flex items-center justify-center gap-6 text-xs">
            <div className="flex items-center gap-1.5">
              <span className="h-3 w-3 rounded-xs bg-slate-300"></span>
              <span className="text-slate-600 font-medium">Scheduled Dues</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="h-3 w-3 rounded-xs bg-emerald-700"></span>
              <span className="text-slate-600 font-medium">Realized Collections</span>
            </div>
          </div>
        </div>
      </div>
