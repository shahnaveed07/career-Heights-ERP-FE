...
export const AdmissionsCrmView = () => {
  ...
  const [convertSource, setConvertSource] = useState('Direct Walk-in');
  const [convertFeesTotal, setConvertFeesTotal] = useState(95e3);
  const [convertFeesPaid, setConvertFeesPaid] = useState(35e3);
  const openConvertModal = (lead) => {
    setConvertModalLead(lead);
    const bId =
      lead.branchId ||
      branches.find((b) => b.name === lead.branchName)?.id ||
      branches[0]?.id ||
      'b-hdw';
    ...
    const matchedBatch =
      batches.find(
        (b) =>
          b.branchId === bId &&
          (b.className === lead.targetCourse ||
            b.name
              .toLowerCase()
              .includes(lead.targetCourse?.toLowerCase() || ''))
      ) ||
      batches.find((b) => b.branchId === bId) ||
      batches[0];
    setConvertBatchId(matchedBatch?.id || '');
    setConvertSource(lead.source || 'Direct Walk-in');
    setConvertFeesTotal(95e3);
    setConvertFeesPaid(35e3);
  };
  const stages = [
    {
      id: 'new',
  ...
  const enrolledCount = filteredEnquiries.filter(
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Track leads across walk-ins, school outreach, and CHTQ talent quest
            examinations.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg border border-slate-200 bg-white p-1">
            <button
              ...
              className={`rounded px-3 py-1 text-xs font-bold transition ${viewMode === 'kanban' ? 'bg-blue-900 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Pipeline Kanban
            </button>
            <button
              onClick={() => setViewMode('table')}
              className={`rounded px-3 py-1 text-xs font-bold transition ${viewMode === 'table' ? 'bg-blue-900 text-white shadow-2xs' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Table View
            </button>
          </div>
      ...
      {viewMode === 'kanban' ? (
        <div className="flex gap-4 overflow-x-auto pb-4">
          {stages.map((stage) => {
            const stageLeads = filteredEnquiries.filter(
              (e) => e.status === stage.id
            );
            return (
              <div
              ...
              >
                ...
                <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-200">
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`h-2.5 w-2.5 rounded-full ${stage.id === 'admission' ? 'bg-emerald-600' : stage.id === 'new' ? 'bg-blue-600' : 'bg-slate-400'}`}
                    />
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            <p className="font-bold text-slate-900">
                              {lead.name || lead.studentName}
                            </p>
                            <p className="text-[10px] text-slate-400">
                              Guardian: {lead.parentName}
                            </p>
                          </div>
                        ...
                        <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[10px]">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <select
                              value={lead.status}
                              onChange={(e) =>
                                updateEnquiryStatus(lead.id, e.target.value)
                              }
                  ...
                  filteredEnquiries.map((lead) => (
                    <tr key={lead.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="py-3 px-4 text-slate-700">
                        {lead.assignedCounsellor || lead.counsellorName}
                      </td>
                      ...
                      <td className="py-3 px-4 text-center">
                        <span
                          className={`inline-block rounded px-2 py-0.5 text-[10px] font-bold ${lead.status === 'admission' ? 'bg-emerald-100 text-emerald-800' : lead.status === 'lost' ? 'bg-slate-200 text-slate-600' : 'bg-blue-100 text-blue-800'}`}
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">
                  Confirm Admission Conversion
                </h3>
                <p className="text-xs text-slate-500">
                  Transform lead to enrolled student record with designated
                  branch &amp; batch
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs space-y-1 border border-slate-200">
              <div className="flex justify-between">
                <span>
                  ...
                  {convertModalLead.name || convertModalLead.studentName}
                </span>
                <span className="text-slate-500">
                  <strong>Counsellor:</strong>{' '}
                  {convertModalLead.assignedCounsellor ||
                    convertModalLead.counsellorName}
                </span>
              </div>
              <p>
                <strong>Parent / Phone:</strong> {convertModalLead.parentName} (
                {convertModalLead.phone})
              </p>
              <p>
                <strong>Target Course:</strong> {convertModalLead.targetCourse}
              </p>
            </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const matchingBatch =
                        batches.find((b) => b.branchId === newBranchId) ||
                        batches[0];
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setConvertFeesTotal(Number(e.target.value))
                    }
                    className="w-full rounded-lg border border-slate-200 bg-white p-2 text-xs text-slate-800 focus:outline-hidden focus:border-blue-900"
                  />
                </div>
            ...
            <p className="mt-3 text-[11px] text-slate-500">
              Generates a permanent sequential Student ID (e.g.{' '}
              <code>CH-2026-XXX</code>), enrolls into the selected batch roster,
              and automatically updates CRM metrics.
            </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs"
                  >
                    <option value="NEET Medical 11th">
                      NEET Medical (Class 11)
                    </option>
                    <option value="NEET Medical 12th">
                      NEET Medical (Class 12)
                    </option>
                    <option value="NEET Dropper Medical">
                      NEET Dropper Repeater
                    </option>
                    <option value="JEE Main & Adv 11th">
                      JEE Main &amp; Adv (Class 11)
                    </option>
                    <option value="JEE Main & Adv 12th">
                      JEE Main &amp; Adv (Class 12)
                    </option>
