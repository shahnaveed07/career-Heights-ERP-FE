...
export const AttendanceManagementView = () => {
  ...
  const [alertSentMessage, setAlertSentMessage] = useState(null);
  useEffect(() => {
    if (
  ...
  const batchSynchronousAttendancePercent =
  ...
  const warningAttendanceStudents = filteredStudents.filter(
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Real-time biometric sync with automated instant parent
            notifications.
          </p>
        </div>
        ...
        <div className="flex rounded-lg border border-slate-200 bg-white p-1">
          <button
            ...
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${activeTab === 'students' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
          >
            Batch Roster
          </button>
          <button
            onClick={() => setActiveTab('faculty')}
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${activeTab === 'faculty' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
          >
            Faculty &amp; Staff
          </button>
          <button
            onClick={() => setActiveTab('low_attendance')}
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${activeTab === 'low_attendance' ? 'bg-red-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
          >
            ...
            <div className="border-b border-slate-200 px-4 py-3 bg-slate-50/70 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-slate-900">
                  {activeBatch?.name} — Daily Register
                </h3>
                <p className="text-[11px] text-slate-500">
                  Room: {activeBatch?.roomNumber} • Mentor:{' '}
                  {activeBatch?.facultyMentor} • {batchStudents.length} Students
                </p>
              </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2.5 px-4 text-center">
                            <span
                              className={`inline-block rounded px-2.5 py-0.5 text-[10px] font-bold ${currentStatus === 'present' ? 'bg-emerald-100 text-emerald-800' : currentStatus === 'absent' ? 'bg-red-100 text-red-800' : currentStatus === 'late' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'}`}
                            >
                              ...
                              {['present', 'absent', 'late', 'leave'].map(
                                (st) => (
                                  <button
                                    key={st}
                                    onClick={() =>
                                      handleToggleStatus(student.id, st)
                                    }
                                    className={`px-2 py-0.5 text-[10px] font-bold rounded transition capitalize ${currentStatus === st ? (st === 'present' ? 'bg-emerald-700 text-white' : st === 'absent' ? 'bg-red-700 text-white' : st === 'late' ? 'bg-amber-600 text-white' : 'bg-blue-700 text-white') : 'text-slate-500 hover:text-slate-900'}`}
                                  >
                                    {st === 'leave'
                                      ? 'Lv'
                                      : st[0].toUpperCase()}
      ...
      {activeTab === 'faculty' && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
          <div className="border-b border-slate-200 px-6 py-4 bg-slate-50/70 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-slate-900">
                Faculty &amp; Academic Staff Biometric Logs
              </h3>
              <p className="text-xs text-slate-500">
                Biometric fingerprint check-ins synchronized with HQ server
              </p>
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-red-200 bg-red-50/50 p-4">
            <div className="flex items-center gap-2 text-red-900 font-bold text-sm">
              ...
              <span>
                Mandatory Intervention List (&lt;75% Attendance Threshold)
              </span>
            </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {criticalAttendanceStudents.map((student) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-slate-600">
                      {student.branchName} • {student.batchName}
                    </td>
