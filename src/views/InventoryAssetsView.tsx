...
export const InventoryAssetsView: React.FC = () => {
  ...
  const [newAsset, setNewAsset] = useState({
    ...
    code: 'AST-AV-2026-09',
    category: 'Projectors & AV' as AssetItem['category'],
    branchId: branches[0]?.id || 'branch-handwara',
    branchName: 'Handwara Campus',
    quantity: 4,
    unit: 'Units',
    locationRoom: 'Smart Lecture Hall 2',
    condition: 'good' as AssetItem['condition'],
    status: 'in_use' as AssetItem['status'],
  ...
  const totalValue = assets.reduce((acc, i) => acc + (i.estimatedValue || 0), 0);
  const maintenanceCount = assets.filter(i => i.status === 'maintenance' || i.condition === 'needs_repair').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Multi-branch tracking of smart interactive boards, student study kits, laboratory instruments and furniture.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-slate-500">Total Tracked Assets</span>
          <div className="mt-1 text-2xl font-black text-slate-900">{assets.length} Lines</div>
          <span className="text-[10px] text-slate-400">Fixed assets &amp; consumables</span>
        </div>

        <div className="rounded-xl border border-blue-200 bg-blue-50/50 p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-blue-800">Total Asset Valuation</span>
          <div className="mt-1 text-2xl font-black text-blue-900">₹{(totalValue / 100000).toFixed(1)}L</div>
          <span className="text-[10px] text-blue-700">Depreciation booked</span>
        </div>

        <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-emerald-800">In Active Deployment</span>
          <div className="mt-1 text-2xl font-black text-emerald-900">
            {assets.filter(i => i.status === 'in_use').length}
          </div>
          <span className="text-[10px] font-bold text-emerald-700">In smart lecture rooms</span>
        </div>

        <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-amber-800">Maintenance Requisitions</span>
          <div className="mt-1 text-2xl font-black text-amber-900">{maintenanceCount}</div>
          <span className="text-[10px] font-bold text-amber-700">Service booked</span>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
        <table className="min-w-full divide-y divide-slate-200 text-xs">
          <thead className="bg-slate-50 text-slate-700 font-bold uppercase text-[10px]">
            <tr>
              <th className="py-3 px-4 text-left">Asset Details &amp; Asset Code</th>
            ...
            {filteredAssets.map(item => (
              <tr key={item.id} className="hover:bg-slate-50 transition">
                <td className="py-3 px-4">
                  ...
                  <p className="font-mono text-[10px] text-blue-900 font-bold">{item.code}</p>
                </td>

                <td className="py-3 px-4">
                  <p className="font-semibold text-slate-800">{item.branchName}</p>
                  <p className="text-[10px] text-slate-400">{item.locationRoom}</p>
                </td>
                ...
                <td className="py-3 px-4 text-center">
                  <span className={`inline-block rounded px-2 py-0.5 text-[10px] font-bold capitalize ${
                    item.condition === 'good' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                  }`}>
                ...
                <td className="py-3 px-4 text-center">
                  <span className={`inline-block rounded px-2.5 py-0.5 text-[10px] font-bold capitalize ${
                    item.status === 'in_use'
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-slate-200">
            <h3 className="font-bold text-slate-900 text-base mb-3">Add Infrastructure Asset</h3>
            <form onSubmit={handleAddSubmit} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setNewAsset({ ...newAsset, locationRoom: e.target.value })}
                    className="w-full rounded-lg border border-slate-300 p-2 text-xs"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Valuation (INR ₹)</label>
                  <input
                    type="number"
