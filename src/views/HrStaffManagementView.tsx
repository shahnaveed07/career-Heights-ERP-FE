    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Faculty roster, biometric attendance compliance, salary ledger and leaves desk.
          </p>
        </div>
        ...
        <div className="flex rounded-lg border border-slate-200 bg-white p-1">
          ...
          <button
            ...
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${
              activeTab === 'leaves' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-emerald-800">Biometric Attendance</span>
          <div className="mt-1 text-2xl font-black text-emerald-900">96.4%</div>
          <span className="text-[10px] font-bold text-emerald-700">Daily punch average</span>
        </div>

        <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-4 shadow-xs">
          <span className="text-[11px] font-semibold text-amber-800">Pending Leave Approvals</span>
          <div className="mt-1 text-2xl font-black text-amber-900">{pendingLeaves.length}</div>
          <span className="text-[10px] font-bold text-amber-700">Requires HR action</span>
        </div>
              ...
              <div key={emp.id} className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs space-y-3">
                ...
                <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-100">
                  <span className="text-slate-500">
                    Salary: <strong className="text-slate-900">₹{(emp.monthlySalary / 1000).toFixed(0)}k/mo</strong>
                  </span>
      ...
      {activeTab === 'leaves' && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
          <div className="p-4 border-b border-slate-200 bg-slate-50/70">
            <h3 className="text-sm font-bold text-slate-900">Faculty &amp; Staff Leave Applications</h3>
            <p className="text-xs text-slate-500">Requires HR or Branch Admin approval to adjust timetable allocations.</p>
          </div>
