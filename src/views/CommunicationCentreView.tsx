...
export const CommunicationCentreView: React.FC = () => {
  const { notifications, sendBroadcastMessage, batches, students } = useErpData();

  const [activeTab, setActiveTab] = useState<'broadcast' | 'logs'>('broadcast');
  const [channel, setChannel] = useState<'sms' | 'whatsapp' | 'push'>('sms');
  const [targetAudience, setTargetAudience] = useState<string>('all_parents');
  ...
  const [broadcastHistory, setBroadcastHistory] = useState<BroadcastRecord[]>([
    {
      ...
      message: 'Monthly Parent-Teacher Mentoring Meet is scheduled for Sunday 10:00 AM at Handwara Main Auditorium.',
    ...
    },
    {
      ...
      message: 'Full Mock Test Phase 1 OMR ranks have been published on your student portal. Review answer keys now.',
  ...
  const handleTemplateChange = (tpl: string) => {
    ...
    if (tpl === 'attendance_alert') {
      setMessageContent('Dear Parent, Your ward was marked PRESENT at Career Heights Handwara Campus for the 08:30 AM Lecture. — Principal CH');
    } else if (tpl === 'fee_reminder') {
      setMessageContent('Dear Parent, Tuition fee installment of ₹25,000 is due on 15 Oct 2026. Avoid late fees by paying online. — Accounts Desk CH');
    } else if (tpl === 'exam_results') {
      setMessageContent('Dear Student, OMR answer keys for Diagnostic Mock Test #3 have been processed. Check your All-India percentile rank in the portal.');
    } else {
      setMessageContent('Important institutional notification from Career Heights Executive Directorate.');
    }
  };
  ...
  const handleSendBroadcast = (e: React.FormEvent) => {
    ...
    const newRecord: BroadcastRecord = {
      id: `b-${Date.now()}`,
      channel,
      target: targetAudience === 'all_parents' ? 'All Enrolled Parents' : 'Target Student Batches',
      message: messageContent,
      template: templateName,
      timestamp: 'Just now',
    ...
    setBroadcastSentNotification(`Broadcast successfully queued across official SMS Gateway & WhatsApp API for delivery.`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Automated SMS gate triggers, WhatsApp notifications, parent alerts, and circulars.
          </p>
        </div>
        ...
        <div className="flex rounded-lg border border-slate-200 bg-white p-1">
          <button
            ...
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${
              activeTab === 'broadcast' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Compose Broadcast
          </button>
          <button
            ...
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${
              activeTab === 'logs' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Delivery Audit Logs
          </button>
      ...
      {activeTab === 'broadcast' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <form onSubmit={handleSendBroadcast} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button
                    ...
                    className={`rounded-xl border p-3 flex items-center justify-center gap-2 font-bold text-xs transition ${
                      channel === 'sms'
                        ? 'border-blue-900 bg-blue-50/50 text-blue-900 shadow-2xs'
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <option value="all_parents">All Enrolled Parents ({students.length})</option>
                    <option value="all_students">All Active Students ({students.length})</option>
                    <option value="faculty">All Teaching Faculty</option>
                    <option value="overdue_parents">Parents with Overdue Fees</option>
                  </select>
                </div>

                <div>
                  <label className="block font-semibold text-slate-700 mb-1">Pre-Approved DLT Template</label>
                  <select
                    value={templateName}
                    onChange={e => handleTemplateChange(e.target.value)}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs"
                  >
                    <option value="attendance_alert">Daily Attendance Check-in</option>
                    <option value="fee_reminder">Due Fee Payment Reminder</option>
                    <option value="exam_results">OMR Rank Publication</option>
                    <option value="custom">General Administrative Circular</option>
                  </select>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-[11px] text-slate-400">{messageContent.length} chars (1 SMS credit)</span>
                </div>
                <textarea
                  rows={4}
              ...
              <div className="flex justify-end pt-2">
                <button
                  type="submit"
                  className="flex items-center gap-2 rounded-lg bg-blue-900 px-5 py-2.5 text-xs font-bold text-white hover:bg-blue-800 shadow-md"
                >
          ...
          <div className="lg:col-span-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col items-center">
            <span className="text-xs font-bold text-slate-600 mb-3">Parent Handset Live Preview</span>
            ...
            <div className="w-64 rounded-3xl border-4 border-slate-800 bg-slate-900 p-2 shadow-xl">
              ...
              <div className="rounded-2xl bg-slate-100 p-3 min-h-[300px] flex flex-col justify-between text-[11px]">
                <div className="space-y-2">
                  <div className="text-center text-[10px] text-slate-400 font-semibold">Today • Carrier SMS</div>
                  <div className="bg-white p-3 rounded-xl rounded-tl-none shadow-xs border border-slate-200 text-slate-800 leading-snug">
                    <p className="font-bold text-blue-900 text-[10px] mb-1">CAREER HEIGHTS HQ</p>
                    <p>{messageContent}</p>
                    <span className="text-[9px] text-slate-400 mt-1 block text-right">09:42 AM • Delivered</span>
                  </div>
                </div>

                <div className="text-center text-[9px] text-slate-400">
                  Secured by Career Heights DLT Header: <strong className="text-slate-600">CRHTS-J&amp;K</strong>
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'logs' && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
          <div className="p-4 border-b border-slate-200 bg-slate-50/70">
            <h3 className="text-sm font-bold text-slate-900">Broadcast Dispatch Ledger &amp; Delivery Receipts</h3>
            <p className="text-xs text-slate-500">Telecom delivery status acknowledgments from SMS and WhatsApp providers.</p>
          </div>
            ...
            {broadcastHistory.map(b => (
              <div key={b.id} className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 hover:bg-slate-50">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-900 text-sm">{b.target}</span>
                  ...
                  <p className="text-[11px] text-slate-400">Template ID: {b.template} • Recipients: {b.recipientsCount}</p>
                </div>
