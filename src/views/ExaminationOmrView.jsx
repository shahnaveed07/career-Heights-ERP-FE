      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Automated OMR answer key scoring, negative marking (+4 / -1), and
            percentile rank lists.
          </p>
        </div>
        ...
        <div className="flex rounded-lg border border-slate-200 bg-white p-1">
          <button
            ...
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${activeTab === 'tests' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
          >
            Scheduled Tests
          </button>
          <button
            onClick={() => setActiveTab('omr_scanner')}
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${activeTab === 'omr_scanner' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
          >
            OMR Scanner Studio
          </button>
          <button
            onClick={() => setActiveTab('ranks')}
            className={`rounded px-3 py-1.5 text-xs font-bold transition ${activeTab === 'ranks' ? 'bg-blue-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
          >
            Rank List &amp; Analytics
          </button>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-bold text-slate-900">
                Career Heights Diagnostic Series
              </h3>
              <p className="text-xs text-slate-500">
                Filter examinations by enrolled course and class wing
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredTests.map((test) => (
                <div
                  ...
                  className={`cursor-pointer rounded-xl border p-5 transition text-left relative ${selectedTestId === test.id ? 'border-blue-900 bg-blue-50/20 shadow-md' : 'border-slate-200 bg-white hover:border-slate-300'}`}
                >
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {test.course} • {test.className} • Max Marks:{' '}
                    {test.totalMarks}
                  </p>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                    <span className="text-slate-500">
                      Results:{' '}
                      <strong>
                        {testResults.filter((r) => r.testId === test.id).length}{' '}
                        Evaluated
                      </strong>
                    </span>
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  ...
                  <span>
                    Optical Mark Recognition (OMR) Rapid Ingestion Studio
                  </span>
                </h3>
              ...
              <p className="text-xs text-slate-500">
                Process scanned physical bubble sheets with automatic answer key
                matching, error-margin validation, and rank compilation.
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="mt-6 w-full max-w-md bg-white p-4 rounded-xl border border-slate-200 text-left">
                ...
                <div className="grid grid-cols-5 gap-2 text-[10px]">
                  {[1, 2, 3, 4, 5].map((q) => (
                    <div
                      key={q}
                      className="flex flex-col items-center gap-1 bg-slate-50 p-1.5 rounded"
                    >
                      <span className="font-bold text-slate-600">Q{q}</span>
                      <div className="flex gap-0.5">
                        <span
                          className={`h-3 w-3 rounded-full border border-slate-400 ${q === 1 || q === 3 ? 'bg-slate-900' : ''}`}
                        />
                        <span
                          className={`h-3 w-3 rounded-full border border-slate-400 ${q === 2 ? 'bg-slate-900' : ''}`}
                        />
                        <span
                          className={`h-3 w-3 rounded-full border border-slate-400 ${q === 4 ? 'bg-slate-900' : ''}`}
                        />
                        <span
                          className={`h-3 w-3 rounded-full border border-slate-400 ${q === 5 ? 'bg-slate-900' : ''}`}
                        />
                      </div>
                    </div>
                  ))}
            ...
            <div className="lg:col-span-5 rounded-xl border border-slate-200 bg-white p-5 space-y-4">
              ...
              <div className="rounded-lg bg-blue-50 p-3 text-xs text-blue-900">
                ...
                <p className="mt-1 text-[11px] text-blue-800">
                  1. Higher Biology/Mathematics score &rarr; 2. Fewer negative
                  questions &rarr; 3. Date of birth seniority.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
          <div className="border-b border-slate-200 px-6 py-4 bg-slate-50/70 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-slate-900">
                  All-India &amp; Branch Merit Rank List
                </h3>
              ...
              <p className="text-xs text-slate-500">
                Diagnostic weak-topic breakdown generated from OMR error
                matrices.
              </p>
            </div>
            <select
                  ...
                  activeTestResults.map((res) => (
                    <tr key={res.id} className="hover:bg-slate-50 transition">
                      <td className="py-3 px-4 text-center">
                        <span
                          className={`inline-flex h-6 w-6 items-center justify-center rounded-full font-black text-xs ${res.instituteRank === 1 ? 'bg-amber-400 text-slate-900 shadow-xs' : res.instituteRank === 2 ? 'bg-slate-300 text-slate-900' : res.instituteRank === 3 ? 'bg-amber-700 text-white' : 'bg-slate-100 text-slate-700'}`}
                        >
                          {res.instituteRank}
                        </span>
                      </td>
                        ...
                        {res.weakTopics.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {res.weakTopics.map((topic, i) => (
                              <span
                                key={i}
                                className="rounded bg-red-50 text-red-700 border border-red-200 px-1.5 py-0.2 text-[10px] font-medium"
                              >
                                {topic}
                              </span>
                            ))}
